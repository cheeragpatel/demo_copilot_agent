...
export class DeliveriesRepository {
    ...
    async exists(id: number): Promise<boolean> {
        try {
            const result = await this.db.get<{count: number}>('SELECT COUNT(*) as count FROM deliveries WHERE delivery_id = ?', [id]);
...
export async function createDeliveriesRepository(isTest: boolean = false): Promise<DeliveriesRepository> {

...
export class OrderDetailsRepository {
    ...
    async findById(id: number): Promise<OrderDetail | null> {
        try {
            const row = await this.db.get<any>('SELECT * FROM order_details WHERE order_detail_id = ?', [id]);
     ...
     */
    async update(id: number, orderDetail: Partial<Omit<OrderDetail, 'orderDetailId'>>): Promise<OrderDetail> {
        try {
    ...
    async exists(id: number): Promise<boolean> {
        try {
            const result = await this.db.get<{count: number}>('SELECT COUNT(*) as count FROM order_details WHERE order_detail_id = ?', [id]);
...
export async function createOrderDetailsRepository(isTest: boolean = false): Promise<OrderDetailsRepository> {
...
export async function getOrderDetailsRepository(isTest: boolean = false): Promise<OrderDetailsRepository> {
    if (!orderDetailsRepo) {

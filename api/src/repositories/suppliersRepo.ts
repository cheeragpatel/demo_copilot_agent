...
export class SuppliersRepository {
    ...
    async exists(id: number): Promise<boolean> {
        try {
            const result = await this.db.get<{count: number}>('SELECT COUNT(*) as count FROM suppliers WHERE supplier_id = ?', [id]);
...
export async function createSuppliersRepository(isTest: boolean = false): Promise<SuppliersRepository> {

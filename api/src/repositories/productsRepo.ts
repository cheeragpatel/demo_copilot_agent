...
export class ProductsRepository {
    ...
    async exists(id: number): Promise<boolean> {
        try {
            const result = await this.db.get<{count: number}>('SELECT COUNT(*) as count FROM products WHERE product_id = ?', [id]);
...
export async function createProductsRepository(isTest: boolean = false): Promise<ProductsRepository> {

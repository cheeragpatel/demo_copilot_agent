...
export class BranchesRepository {
    ...
    async exists(id: number): Promise<boolean> {
        try {
            const result = await this.db.get<{count: number}>('SELECT COUNT(*) as count FROM branches WHERE branch_id = ?', [id]);
...
export async function createBranchesRepository(isTest: boolean = false): Promise<BranchesRepository> {

...
export class HeadquartersRepository {
    ...
    async findById(id: number): Promise<Headquarters | null> {
        try {
            const row = await this.db.get<any>('SELECT * FROM headquarters WHERE headquarters_id = ?', [id]);
     ...
     */
    async update(id: number, headquarters: Partial<Omit<Headquarters, 'headquartersId'>>): Promise<Headquarters> {
        try {
    ...
    async exists(id: number): Promise<boolean> {
        try {
            const result = await this.db.get<{count: number}>('SELECT COUNT(*) as count FROM headquarters WHERE headquarters_id = ?', [id]);
...
export async function createHeadquartersRepository(isTest: boolean = false): Promise<HeadquartersRepository> {

...
export class OrderDetailDeliveriesRepository {
    ...
    async findAll(): Promise<OrderDetailDelivery[]> {
        try {
            const rows = await this.db.all<any>('SELECT * FROM order_detail_deliveries ORDER BY order_detail_delivery_id');
    ...
    async findById(id: number): Promise<OrderDetailDelivery | null> {
        try {
            const row = await this.db.get<any>('SELECT * FROM order_detail_deliveries WHERE order_detail_delivery_id = ?', [id]);
     ...
     */
    async create(orderDetailDelivery: Omit<OrderDetailDelivery, 'orderDetailDeliveryId'>): Promise<OrderDetailDelivery> {
        try {
     ...
     */
    async update(id: number, orderDetailDelivery: Partial<Omit<OrderDetailDelivery, 'orderDetailDeliveryId'>>): Promise<OrderDetailDelivery> {
        try {
            const { sql, values } = buildUpdateSQL('order_detail_deliveries', orderDetailDelivery, 'order_detail_delivery_id = ?');
    ...
    async delete(id: number): Promise<void> {
        try {
            const result = await this.db.run('DELETE FROM order_detail_deliveries WHERE order_detail_delivery_id = ?', [id]);
    ...
    async exists(id: number): Promise<boolean> {
        try {
            const result = await this.db.get<{count: number}>('SELECT COUNT(*) as count FROM order_detail_deliveries WHERE order_detail_delivery_id = ?', [id]);
...
export async function createOrderDetailDeliveriesRepository(isTest: boolean = false): Promise<OrderDetailDeliveriesRepository> {
...
export async function getOrderDetailDeliveriesRepository(isTest: boolean = false): Promise<OrderDetailDeliveriesRepository> {
